...
export class TazaPayMCPClient {
  ...
  async isConfigured(): Promise<boolean> {
    try {
      ...
      if (!mcpConfig?.servers?.[TAZAPAY_CONFIG.MCP_SERVER_NAME]) {
        return false;
      }

      const tazapayConfig = vscode.workspace.getConfiguration('tazapay');
      const apiKey = tazapayConfig.get<string>('apiKey');
      const secretKey = tazapayConfig.get<string>('secretKey');
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      this.mcpProcess = spawn('docker', [
        'run', '--rm', '-i',
        '-e', `${TAZAPAY_CONFIG.ENV_VARS.API_KEY}=${apiKey}`,
        '-e', `${TAZAPAY_CONFIG.ENV_VARS.API_SECRET}=${secretKey}`,
   ...
   */
  private handleMCPOutput(data: string): void {
    try {
   ...
   */
  findRelevantTools(query: string): MCPTool[] {
    const lowerQuery = query.toLowerCase();
    
    // First try exact tool name matches
    const exactMatches = this.tools.filter(tool => 
      lowerQuery.includes(tool.name.toLowerCase()) ||
    ...
    const descriptionMatches = this.tools.filter(tool => 
   ...
   */
  private isQueryRelevantToTool(query: string, tool: MCPTool): boolean {
